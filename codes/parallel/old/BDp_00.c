...
void main(int argc, char **argv){
  ...
  int Tint;						/* Snapshot measurement interval */
  ...
  scanf("Density (phi): %f\n", & fi);
  scanf("Simulation time (time units): %d\n", & Tmax);
  ...
  scanf("Time Step: %f\n", & dt);
  scanf("Snap measuring interval: %d\n", & Tint);
  ...
  printf("Simulation time (time units): %d\n",  Tmax);
  ...
  printf("Time Step: %f\n", dt);
  printf("Snap measuring interval: %d\n", Tint);
  ...
  /* TIME LOOP - DYNAMICS*/
      ...
      /*DEBUG*/ if (DEBUG==3) {getchar(); printf("\n\n\nSTARTING TIME LOOP\n\n\n");}

  for (step=0; step < (Tmax/dt); step++) {
    ...
    for (k=0; k<N; k++) {
      ...
      if ((step+1)%(int)(Tmax/(measures*dt))==0 && step>0) {
    ...
    int tag1, tag2, idist;
    float dist;
    if ((step+1)%(int)(Tmax/(measures*dt)) == 0 && step>0) {				/* Only at selected time steps */
      if (DEBUG==3) {
        printf("\n\n\nState of the system:\n");
      ...
      fprintf(corr, "%f	", (step+1)*dt);				/* Save correlations */
    ...
    if ((step+1)%(int)(Tmax/(measures*dt)) == 0 && step>0)
    {
    	...
    	E=0;							/* reset energy */
      	for (sn=0; sn<N; sn++) 
      	{
        	E+=0.5*pow(trueDirections[sn],2)+0.5*V[sn];	/*DEBUG*/ if (DEBUG==3) {printf("  Particle %d - v=%.3f - V=%.3f - E=%.3f\n",sn,trueDirections[sn],V[sn],0.5*pow(trueDirections[sn],2)+V[sn]);}
      	}

      	/* compute and normalize E */
      	E=E/(float)N;									/*DEBUG*/ if (DEBUG==3) {printf("\nEnergy = %.5f\n",E); getchar();}
    ...
    /* snap initial times */
    int saveStep;
    if (Tint >= measures){saveStep = Tint/(measures*dt);}
    else if (Tint < measures){saveStep=1/dt;}

  	if ( (step+1)%saveStep == 0 && step+1 <= Tint/dt ) 
  	{
  	...
  	/* snap final times */
  	else if ((step+1)%saveStep==0 && step+1>=(Tmax-Tint)/dt ) 
  	{
