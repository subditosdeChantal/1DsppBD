...
int main(int argc, char **argv){
	...
	int clusterArray[(int)(Npart/2)][Npart] = {0};
	...
	float CoM[Npart] = {0}; // center of mass
	int sizes[Npart]= {0}; // size of cluster
	float CSD[Npart]= {0}; // cluster size distribution
	float probJump[Npart]= {0}; // jumping prob
	int dirclust[Npart]= {0};  // jumping direction
	int Nclust = 0;
	...
	/* READ FROM FILE */
		...
		sprintf(bufferframe0, "frame0_a%.3f_f%.3f_L%.5d_D%05.2f_Fp%.2f_eps%07.4f_CMOB%d.dat", alpha, fi, L, Dt, Fp, epsilon, CMOB);
	...
	for (int j = 0; j < N; ++j)
	{
		...
		if ( !visited[j] )
		{
	        ...
	        // printf("\n\nCluster %d of size %d:: auxL: %d - particlesOrder[auxL]: %d\n", i, l, auxL, particlesOrder[auxL]);
	        // printf("Cluster %d of size %d:: auxR: %d - particlesOrder[auxR]: %d\n", i, l, auxR, particlesOrder[auxR]);
	        ...
	        while(inclustL)
	        {
	        	...
	        	if (distLneigh < cluster_cutoff)
	        	{
					...
					l++;
					// printf("Cluster %d of size %d:: auxL: %d - particlesOrder[auxL]: %d\n", i, l, auxL, particlesOrder[auxL]);
